    ...
    S: Eq + Ord + Hash + Clone + Debug,
{
    pub fn new(src: &Vec<S>) -> HuffEncoder<S> {
        ...
        HuffEncoder { frequency, mapping, hufftree }
    }
    ...
    fn build_huffman_tree(freqs: &HashMap<S, usize>) -> HuffTree<S> {
        ...
        */
        ...
        loop {
            let t1 = trees.pop().unwrap(); // First pop is always good
            let t2 = match trees.pop() {
                // If this fails, we can bail
                Some(tree) => tree,
                None => return t1,
            };
...
mod tests {
    ...
    #[test]
     fn simple_map() {
        ...
        let answer : HashMap<char,BitVec> = vec![('c',bitvec![0,0])
                         ,('b',bitvec![0,1])
                         ,('a',bitvec![1])].into_iter().collect();
