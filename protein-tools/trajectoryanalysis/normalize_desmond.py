...
def normalize_topofile(topofile):
    ...
    exitstatus = subprocess.run(
        ["vmd", "-dispdev", "text", "-e", convertscript, "-args", topofile, outfile]
    )
...
def normalize_desmond_data(source_dir, target, compress=False):
    ...
    tmptld = (
        tempfile.mkdtemp()
    )  # Keep so we can have a nice neat subdirectory name in tar
    ...
    if num_trj != 1:
        raise ValueError(
            "I expected exactly 1 trajectory folder, but I found "
            + str(num_trj)
            + ": "
            + str(glob.glob(trj_pattern))
        )
    ...
    if not compress:
        ...
        exitstatus = subprocess.run(
            ["tar", "--zstd", "-cvf", target, "-C", os.path.dirname(tmpdir), name]
        )
...
if __name__ == "__main__":
    if sys.argv[1] == "-h" or sys.argv[1] == "--help" or len(sys.argv) < 2:
        print("Usage: {} <input cms> [output pdb]".format(sys.argv[0]))
