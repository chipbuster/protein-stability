...
function test_flip_point()
    ...
    for i in 1:1000
        ...
        res = reflect_point_across_line(testpt, (e1,e2))
        (x,y) = (res[1], res[2])
        expected = [-y,-x]
        if norm(testpt - expected) > 1e-6
            println("y=-x test failure with norm = $(norm(testpt - expected))")
            println("Input was $(testpt), Result was $(res), expected $(expected)")
            return false
        end
    end
