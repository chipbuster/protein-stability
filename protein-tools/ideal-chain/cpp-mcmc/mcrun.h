...
struct MCRunSettings {
  ...
  MCRunSettings(int numAngles, int64_t numSteps, int64_t skipsPerStep,
                double lo, double hi)
      : numAngles{numAngles}, numSteps{numSteps},
        gaussWidth{(hi - lo) / (numAngles * numAngles)},
...
/** A class which is used to amortize H5Write costs. It assumes the data is added
 * one vector at a time into the dataset. It uses an internal buffer to avoid
 * calling into HDF5 all the time. Once the buffer is exhausted, it flushes all
 * of its data into the specified HDF5 dataset. */
class MCBuffer {
  MatrixXd buffer; // Used to buffer writes so we don't call into HDF5 every iter
