 ...
 * designed as an alternative to std::xoroshiro128plus.
 * Taken from
 * https://gist.github.com/siebenschlaefer/bfe750df9b7104ba4eed874223b8ab1f
 *
 * Usage:
...
template <class Seq, class = void> struct is_seed_sequence : std::false_type {};

template <class Seq>
struct is_seed_sequence<
    Seq, void_t<
             // typename Seq::result_type,
             decltype(std::declval<Seq>().generate(std::declval<uint32_t *>(),
...
public:
  ...
  template <typename SeedSequence> xoroshiro128plus(SeedSequence seq) {
    seed(seq);
  }
  void seed() {
  ...
  template <typename SeedSequence,
            typename = typename std::enable_if<
                is_seed_sequence<SeedSequence>::value>::type>
