...
public:
  PhysicsHook()
      : sim_thread(NULL), please_pause(false), please_die(false),
        running(false) {}

  virtual ~PhysicsHook() { killSimThread(); }
  ...
  /*
   * Runs once when the simulation is initialized, and again each time the user
   * resets the simulation.
   */
  virtual void initSimulation() = 0;

  /*
   * Called every once in a while (and always before every simulation step) even
   * if the simulation is paused. Use this to update the visualization in
   * response to user input etc.
   */
  virtual void tick(){};

  /*
   * Takes one simulation "step." You can do whatever you want here, but the
   * granularity of your computation should be small enough that the user can
   * view/pause/kill the simulation at interactive rates. This method *must* be
   * thread-safe with respect to renderRenderGeometry() (easiest is to not touch
   * any rendering data structures at all).
   */
  virtual bool simulateOneStep() = 0;

  /*
   * Update the rendering data structures here. This method will be called in
   * alternation with simulateOneStep(). This method blocks rendering in the
   ...
   * respect to simulateOneStep(). This method runs in the same thread as the
   ...
   * This method is called in the *rendering thread*. If you need to make
   * changes to the simulation state, you should stash the mouse click in a
   * message queue and deal with it in the simulation thread.
   */
  ...
  /*
   * Resets the simulation (and leaves it in a paused state; call run() to start
   * it).
   */
  void reset() {
    killSimThread();
  ...
  /*
   * Pause a running simulation. The simulation will pause at the end of its
   * current "step"; this method will not interrupt simulateOneStep
   * mid-processing.
   */
  void pause() {
    status_mutex.lock();
  ...
  bool showSimButtons() { return show_sim_buttons; }
